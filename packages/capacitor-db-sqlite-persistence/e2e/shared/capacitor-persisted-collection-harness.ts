...
export type CapacitorPersistedCollectionHarnessConfig = E2ETestConfig
...
export async function createCapacitorPersistedCollectionHarnessConfig<
...
>(options: {
  ...
  const eagerUsers = createPersistedCollection<User, TDatabase>(
    database,
    `capacitor-persisted-users-eager-${suiteId}`,
    `eager`,
    createPersistence,
  )
  const eagerPosts = createPersistedCollection<Post, TDatabase>(
    database,
    `capacitor-persisted-posts-eager-${suiteId}`,
    `eager`,
    createPersistence,
  )
  const eagerComments = createPersistedCollection<Comment, TDatabase>(
    database,
    `capacitor-persisted-comments-eager-${suiteId}`,
    `eager`,
    createPersistence,
  )
  ...
  const onDemandUsers = createPersistedCollection<User, TDatabase>(
    database,
    `capacitor-persisted-users-ondemand-${suiteId}`,
    `on-demand`,
    createPersistence,
  )
  const onDemandPosts = createPersistedCollection<Post, TDatabase>(
    database,
    `capacitor-persisted-posts-ondemand-${suiteId}`,
    `on-demand`,
    createPersistence,
  )
  const onDemandComments = createPersistedCollection<Comment, TDatabase>(
    database,
    `capacitor-persisted-comments-ondemand-${suiteId}`,
    `on-demand`,
    createPersistence,
  )
